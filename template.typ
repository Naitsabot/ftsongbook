...
#let vers(body) = {
    ...
    grid(
        ...
        column-gutter: 0.5em,
        align: (top, left),
        text(font: config.song-text-font, size: config.song-text-size, weight: config.song-text-weight)[#context counter("verse").display().],
        ...
        [
            #layout(size => {
                ...
                let subtext-content = if subtext != none {
                    text(font: config.subtext-font, size: config.subtext-text-size, weight: config.subtext-text-weight, style: "italic")[#subtext]
                } else { none }
